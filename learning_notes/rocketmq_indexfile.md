...
也就是约 400 MB。`maxIndexNum` 是固定值 2000 万，只调大 `maxHashSlotNum` 并不会让它自动跟随。实现把索引下标 0 作为无效值，因此实际可用索引数约为 `indexNum - 1`。
...
注意：索引项只保存哈希值，不保存原始 Key。不同字符串如果发生 Java hash 冲突，存在理论上的误匹配可能，这是该实现的已知局限。
